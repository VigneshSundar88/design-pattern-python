...
class PipelineBuilder:
    ...
    def main():
        ...
        logger = io_module.LoggerProvider.get_logger(driver_obj.spark, driver_obj.__class__.__name__)
        input_args = sys.argv
        ...
        driver_obj._parse_job_config(driver_obj, logger, applicationProperties)

    def _parse_job_config(self, obj_val, logger, applicationProperties):
        ...
        if(obj_job_config.get(source_system) == "ORACLE"):
            audit_logger_obj = audit_logger.audit_logger(spark=obj_val.spark, class_name=obj_job_config.get(Constants.ORACLE_CN), obj_config=obj_job_config)
